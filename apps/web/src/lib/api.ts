...
export const api = {
  ...
  smtpRelays: () => request<ListResponse<SMTPRelay>>("/api/admin/smtp-relays"),
  createSmtpRelay: (payload: SMTPRelayPayload) => request<SMTPRelay>("/api/admin/smtp-relays", { method: "POST", body: JSON.stringify(payload) }),
  updateSmtpRelay: (id: string, payload: SMTPRelayPayload) => request<SMTPRelay>(`/api/admin/smtp-relays/${id}`, { method: "POST", body: JSON.stringify(payload) }),
  deleteSmtpRelay: (id: string) => request<{ ok: boolean }>(`/api/admin/smtp-relays/${id}`, { method: "DELETE" }),
  testSmtpRelay: (id: string, to: string) => request<{ ok: boolean }>(`/api/admin/smtp-relays/${id}/test`, { method: "POST", body: JSON.stringify({ to }), timeoutMs: MAIL_DELIVERY_TIMEOUT_MS }),
  deliverabilitySettings: () => request<DeliverabilitySettings>("/api/admin/deliverability-settings"),
  updateDeliverabilitySettings: (payload: DeliverabilitySettingsPayload) => request<DeliverabilitySettings>("/api/admin/deliverability-settings", { method: "POST", body: JSON.stringify(payload) }),
